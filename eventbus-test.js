...
(function () {
  ...
  window.EventBusTestV5 = {
    ...
    async testSubscribe() {
      ...
      try {
        let received = false;
        const unsubscribe = EventBus.subscribe('TEST_SUB_V5', () => { received = true; });
    ...
    async testMessageTemplates() {
      ...
      try {
        ...
        return { name: '消息模板', passed: templates.includes('USER_ACTION') && message.userId === '123' };
      } catch (error) {
    ...
    showFullStatus() {
      ...
      Object.entries(state.circuitBreakers).forEach(([type, breaker]) => {
        const statusSymbol = breaker.state === 'closed' ? '🟢' : breaker.state === 'open' ? '🔴' : '🟡';
    ...
    showMemoryReport() {
      ...
      if (report.error) {
        ...
        if (report.memory) {
          ...
          console.log('  趋势:', report.memory.trend > 0 ? '+' : '', (report.memory.trend / 1024).toFixed(2), 'KB');
        }
