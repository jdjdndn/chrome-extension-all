...
describe('ResourcePreloader', () => {
  ...
  it('should skip initialization if ResourcePreloader already exists', async () => {
    ...
    expect(logSpy).toHaveBeenCalledWith('[ResourcePreloader] 已存在，跳过初始化')
    logSpy.mockRestore()
  })
  ...
  describe('init()', () => {
    it('should not process resources if disabled', async () => {
      ...
      const logSpy = vi.spyOn(console, 'log')
      instance.init()
      expect(logSpy).toHaveBeenCalledWith('[ResourcePreloader] 模块已禁用')
      logSpy.mockRestore()
    })
    ...
    it('should process resources if enabled', async () => {
      ...
      const logSpy = vi.spyOn(console, 'log')
      instance.init()
      expect(logSpy).toHaveBeenCalledWith('[ResourcePreloader] 初始化完成')
      logSpy.mockRestore()
    })
  })
  ...
  describe('_preconnectCDNs', () => {
    it('should add dns-prefetch and preconnect links for valid CDNs', async () => {
      ...
      ;(window as any).CDNMappings = {
        CDN_SOURCES: [{ id: 'bootcdn', baseUrl: 'https://cdn.bootcdn.net/test/' }],
      }
    ...
    it('should handle invalid URLs gracefully', async () => {
      ...
      ;(window as any).CDNMappings = {
        CDN_SOURCES: [{ id: 'invalid-cdn', baseUrl: 'not-a-valid-url' }],
      }
    ...
    it('should not add preconnect for non-priority CDNs', async () => {
      ...
      ;(window as any).CDNMappings = {
        CDN_SOURCES: [{ id: 'unimportant-cdn', baseUrl: 'https://cdn.example.com' }],
      }
  ...
  describe('_processExistingResources', () => {
    it('should preload critical JS scripts if preloadJS is true', async () => {
      ...
      expect(instance._isCritical).toHaveBeenCalledWith('https://example.com/critical.js', 'js')
    })
    ...
    it('should preload critical CSS links if preloadCSS is true', async () => {
      ...
      expect(instance._isCritical).toHaveBeenCalledWith('https://example.com/critical.css', 'css')
    })
