...
(function() {
  ...
  class OptimizationAdvisor {
    ...
    analyze(selector, element = null) {
      ...
      if (!selector) {
        this.suggestions.push(this._createSuggestion({
          type: 'error',
          severity: 'critical',
          message: '选择器为空',
          fix: '请提供有效的选择器',
    ...
    _analyzeStability(selector, element) {
      ...
      // 自动生成的 class
      const unstableClass = selector.match(/\.([a-zA-Z_-][a-zA-Z0-9_-]*)/g)?.find(cls =>
    ...
    _checkBestPractices(selector, element) {
      ...
      if (classMatch) {
        for (const cls of classMatch) {
          ...
          const isSemantic = BEST_PRACTICES.semanticClassPatterns.some(pattern => pattern.test(className));
    ...
    _generateAlternatives(element, currentSelector) {
      ...
      const classes = (element.className || '').split(' ').filter(c =>
        c && !BEST_PRACTICES.unstableClassPatterns.some(pattern => pattern.test(c))
      );
