...
(function() {
  ...
  _analyzeBasic(element) {
    ...
    if (element.className && typeof element.className === 'string') {
      const classes = element.className.split(' ').filter(c =>
        c && !/^(css-|styled-|sc-|js-|_|__|Mui|jss|ep-)/.test(c)
      );

      const semanticClasses = classes.filter(c =>
        /^(btn|button|link|nav|menu|item|card|list|form|input|container|wrapper|header|footer|content|title|text|icon|active|disabled|selected|hidden|show|fade|slide|modal|dropdown|tooltip|badge|label|error|success|warning)/i.test(c)
      );
    ...
    // 属性推荐
    const stableAttrs = ['type', 'role', 'data-type', 'data-role', 'data-testid', 'data-test', 'name'];
  ...
  _analyzeContext(element) {
    ...
    if (parent.className && typeof parent.className === 'string') {
      const parentClass = parent.className.split(' ').find(c =>
        c && !/^(css-|styled-|sc-|js-|_|__|Mui|jss|ep-)/.test(c)
      );
      if (parentClass) {
        recommendations.push({
  ...
  getRecommendedStrategy() {
    ...
    switch (maxType) {
      case 'id': return 'prefer-id';
      case 'data-attr': return 'prefer-attribute';
      default: return 'prefer-class';
    }
  }
