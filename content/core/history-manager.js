      ...
      try {
        ...
        if (snapshot && typeof Services !== 'undefined') {
          ...
          switch (record.action) {
            case 'SET_SETTINGS':
              await Services.settings.set(Object.keys(snapshot.snapshot)[0], Object.values(snapshot.snapshot)[0]);
              break;
            case 'UPDATE_HIDE_ELEMENTS':
              await Services.hideElements.update(snapshot.snapshot.enabled, snapshot.snapshot.selectors);
              break;
            case 'ADD_KEYWORD':
              // 移除关键词
              break;
