...
(function () {
  ...
  async function sendToBackground(type, data, options = {}) {
    ...
    try {
      const result = retry
        ? await MessageRetry.execute(send)
        : await send();
  ...
  const Services = {
    ...
    domain: {
      ...
      async checkBlocked(currentDomain, requestDomain) {
        return sendToBackground('CHECK_DOMAIN_BLOCKED', {
          currentDomain,
          requestDomain
        }, { cache: false });
      },
      ...
      async registerBlocked(domain, blockedDomains) {
        MessageCache.invalidate('CHECK_DOMAIN');
        return sendToBackground('REGISTER_BLOCKED_DOMAINS', {
          domain,
          blockedDomains
        }, { cache: false });
      }
    ...
    settings: {
      ...
      async set(key, value) {
        ...
        return sendToBackground('STORAGE_SET', {
          data: { [key]: value },
          area: 'sync'
        }, { cache: false });
      },
