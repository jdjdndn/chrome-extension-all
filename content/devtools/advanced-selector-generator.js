...
(function () {
  ...
  const SKIP_ATTRS = new Set([
    ...
    'class', 'id', 'style', 'aria-label', 'aria-describedby', 'aria-labelledby',
    'title', 'alt', 'placeholder', 'value', 'href', 'src', 'data-tooltip',
  ...
  function tryClassPattern(elements, common) {
    ...
    if (common.classes.length >= 2) {
      const sel = tag + '.' + common.classes.slice(0, 2).map(c => CSS.escape(c)).join('.');
      const matchCount = testSelector(sel).length;
      results.push({ selector: sel, matchCount });
    }
    ...
    if (common.classes.length > 0 && common.attributes.length > 0) {
      const sel = tag + '.' + CSS.escape(common.classes[0]) +
        '[' + CSS.escape(common.attributes[0].name) + '="' + CSS.escape(common.attributes[0].value) + '"]';
      const matchCount = testSelector(sel).length;
      results.push({ selector: sel, matchCount });
    }
