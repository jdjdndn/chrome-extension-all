...
(function () {
  ...
  class ElementPicker {
    ...
    onHover(event) {
      ...
      // 忽略高亮层自身
      if (element === this.highlightOverlay || element === this.sizeTooltip) {
        return
      }
      if (element === this.breadcrumbEl || element === this.levelIndicator) {
        return
      }
    ...
    onMouseDown(event) {
      ...
      // 忽略高亮层自身
      if (event.target === this.highlightOverlay || event.target === this.sizeTooltip) {
        return
      }
     ...
     */
    onMouseMove(event) {
      if (!this.isActive || !this.isBoxSelecting) {
        return
      }
    ...
    generateSelector(element) {
      ...
      if (classes.length >= 2) {
        ...
        for (let i = candidates.length - 2; i < candidates.length; i++) {
          if (isExactMatch(candidates[i], element)) {
            return candidates[i]
          }
        }
      }
      ...
      for (let depth = 2; depth <= 5; depth++) {
        ...
        if (isExactMatch(selector, element)) {
          return selector
        }
        ...
        if (result.contains && result.count > 1 && result.count <= 5) {
          ...
          if (classes.length > 1 && !lastPart.includes(':nth')) {
            ...
            if (isExactMatch(enhancedSel, element)) {
              return enhancedSel
            }
          }
    ...
    async getMergedSelector() {
      ...
      if (this.selectedElements.length === 1) {
        return this.selectedElements[0].selector
      }
    ...
    _extractElementFeatures(element) {
      const getValidClasses = (node) => {
        ...
        return node.className
          ...
          .filter((c) => {
            ...
            if (/^(css-|styled-|sc-|js-|_|__|Mui|jss|css_|_)/.test(c) || c.length > 30) {
              return false
            }
            return /^[a-zA-Z][a-zA-Z0-9_-]*$/.test(c)
          })
      ...
      const getValidAttributes = (node, forMerge = false) => {
        ...
        for (const attr of node.attributes) {
          // 如果是用于合并，只允许确定性属性
          if (forMerge && !STABLE_ATTRS.has(attr.name)) {
            continue
          }
          if (
      ...
      const getPseudoInfo = (node) => {
        const parent = node.parentElement
        if (!parent) {
          return { nthOfType: 0, isFirst: false, isLast: false, isOnly: false }
        }
      ...
      const getPath = (node) => {
        ...
        while (current && current !== document.body) {
          ...
          } else if (pseudo.isLast) {
            selector += ':last-child'
          } else if (pseudo.isOnly) {
            selector += ':only-child'
          } else if (pseudo.nthOfType > 0) {
            selector += `:nth-child(${pseudo.nthOfType})`
          }
    ...
    _decomposeFeatures(element) {
      ...
      const getAttrs = (node) => {
        ...
        for (const attr of node.attributes) {
          ...
          if (!attr.value || attr.value.length > 80 || /^\d+$/.test(attr.value)) {
            continue
          }
          attrs.push({ name: attr.name, value: attr.value })
        }
      ...
      const getPseudos = (node) => {
        ...
        if (sameType.length > 1) {
          if (idx === 0) {
            pseudos.push(':first-of-type')
          }
          if (idx === sameType.length - 1) {
            pseudos.push(':last-of-type')
          }
          if (idx > 0) {
            pseudos.push(':nth-of-type(' + (idx + 1) + ')')
          }
        }
        if (siblings.length > 1) {
          if (allIdx === 0) {
            pseudos.push(':first-child')
          }
          if (allIdx === siblings.length - 1) {
            pseudos.push(':last-child')
          }
          if (allIdx > 0) {
            pseudos.push(':nth-child(' + (allIdx + 1) + ')')
          }
        }
    ...
    _compoundCandidates(feat, maxParts) {
      ...
      if (tag) {
        if (feat.id) {
          add(tag + '#' + CSS.escape(feat.id))
        }
        for (const cls of classes) {
          add(tag + '.' + CSS.escape(cls))
        }
        for (const attr of attrs) {
    ...
    *generateCandidates(common, allFeatures, elements) {
      ...
      for (const tSel of targetPool.slice(0, 5)) {
        ...
        for (const mod of modifierPool) {
          if (tSel === tag || tSel.startsWith(tag + '.')) {
            targetVariants.push(tSel + mod)
          }
        }
      }
      ...
      for (const sSel of siblingPool.slice(0, 3)) {
        for (const mod of modifierPool.slice(0, 2)) {
          sibVariants.push(sSel + mod)
        }
      }
    ...
    _isExactMatchForAll(selector, targetElements) {
      try {
        const found = document.querySelectorAll(selector)
        if (found.length !== targetElements.length) {
          return false
        }
