...
function initWidenPage() {
  ...
  // 需要排除的元素（导航栏、页脚、侧边栏等）
  const EXCLUDE_TAGS = new Set(['NAV', 'HEADER', 'FOOTER', 'ASIDE', 'SCRIPT', 'STYLE', 'NOSCRIPT', 'TEMPLATE', 'SVG', 'IFRAME']);
  // 排除的选择器
  const EXCLUDE_SELECTORS = [
    ...
    '.header', '.footer', '.toolbar', '.breadcrumb',
    ...
    function scan(root, depth) {
      ...
      for (const el of root.children) {
        ...
        if (hasMaxWidthConstraint || hasFixedWidth) {
          // 检查是否居中
          const isCentered = style.marginLeft === 'auto' && style.marginRight === 'auto'
  ...
  function buildSelector(el) {
    ...
    if (el.className && typeof el.className === 'string') {
      const cls = el.className.trim().split(/\s+/).find(c => c && c.length < 30);
  ...
  function inject(width) {
    ...
    console.log('[通用脚本] 页面宽度扩展已生效 (' + width + 'vw)，覆盖 ' + elements.length + ' 个容器:', elements.map(e => e.selector).join(', '));
  }
