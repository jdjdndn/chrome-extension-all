    ...
    window.PanelPositionManager = {
        ...
        if (pageLayout.mainElement) {
          ...
          if (rightSpaceUsable < iconWidth + edgeMargin) {
            ...
            if (pageLayout.leftSpace >= iconWidth + edgeMargin * 2) {
              ...
              iconOnLeftSide = true
              const iconLeftEdge = Math.max(
                edgeMargin,
                Math.floor((pageLayout.leftSpace - iconWidth) / 2)
              )
              iconRight = vw - iconLeftEdge - iconWidth
              logger.debug(
                ` 右侧空白不足(${Math.round(rightSpaceUsable)}px)，icon移至左侧: right=${Math.round(iconRight)}`
              )
            } else {
              logger.debug(
                ` 右侧空白不足(${Math.round(rightSpaceUsable)}px)，左侧也不足，icon保持默认位置`
              )
            }
          }
        }
        ...
        if (pageLayout.mainElement && pageLayout.rightSpace > scrollbarAwareEdge) {
          ...
          if (iconOnLeftSide) {
            ...
            if (rightSpace >= minPanelWidth + scrollbarAwareEdge) {
              ...
              logger.debug(
                ` icon在左侧，panel独立定位: panelRight=${Math.round(panelRight)}, width=${Math.round(panelWidth)}`
              )
            }
          } else {
            // icon 在右侧，原有逻辑
            ...
            if (rightSpace >= minRequiredSpace) {
              ...
              panelWidth = Math.min(
                maxPanelWidth,
                rightSpace - iconWidth - panelAvoidGap - scrollbarAwareEdge
              )
              panelRight = scrollbarAwareEdge + iconWidth + panelAvoidGap
              logger.debug(
                ` 右侧空间足够: panelRight=${Math.round(panelRight)}, width=${Math.round(panelWidth)}`
              )
              ...
              if (pageLayout.leftSpace >= minPanelWidth + edgeMargin) {
                ...
                logger.debug(
                  ` panel放左侧: panelRight=${Math.round(panelRight)}, width=${Math.round(panelWidth)}`
                )
              } else {
                ...
                panelRight = scrollbarAwareEdge + iconWidth + panelAvoidGap
                logger.debug(
                  ` panel调整宽度: panelRight=${Math.round(panelRight)}, width=${Math.round(panelWidth)}`
                )
              }
            }
          }
            ...
            if (isCollapsed) {
              ...
              if (pos) {
                ...
                if (this.config.enableSmartAvoidance && !pos.isUserCustomized) {
                  ...
                  const currentSeverity = this.evaluatePanelPosition(
                    pos.right,
                    pos.top,
                    panelWidth,
                    panelHeight,
                  ...
                  if (
                    bestSeverityScore < currentSeverityScore ||
                    (bestSeverityScore === currentSeverityScore &&
                      bestPos.overlapRatio < currentSeverity.overlapRatio)
                  ) {
                    finalRight = bestPos.right
                    finalTop = bestPos.top
                    logger.debug(
                      ` ${c.id} panel位置优化: (${pos.right}, ${pos.top}) -> (${finalRight}, ${finalTop}), severity: ${currentSeverity.severity} -> ${bestPos.severity}`
                    )
                  }
                }
      ...
      checkComponentOcclusion(componentId) {
        ...
        if (c.iconEl && c.iconEl.dataset.ycType) {
          ...
          if (iconBounds) {
            const iconOcclusion = this.detectOcclusionInArea(iconBounds, excludeElements, 'icon')
        ...
        if (c.panelEl && c.panelEl.dataset.ycType && !this.collapsedStates[c.id]) {
          ...
          if (panelBounds) {
            const panelOcclusion = this.detectOcclusionInArea(panelBounds, excludeElements, 'panel')
      ...
      detectPageFixedElementOcclusions(options = {}) {
        ...
        // 按严重程度排序
        const severityOrder = { critical: 0, high: 1, medium: 2, low: 3, none: 4 }
        results.elements.sort(
          (a, b) => severityOrder[a.occlusion.severity] - severityOrder[b.occlusion.severity]
        )
      ...
      checkElementOcclusion(el, excludeElements = new Set()) {
        ...
        // 采样检测遮挡内容
        const sampleResult = this.sampleOccludedElements(
          rect.left,
          rect.top,
          rect.width,
          rect.height,
      ...
      findBestPosition(panelWidth, panelHeight, preferredRight, preferredTop) {
        ...
        for (
          ...
          right <= vw - panelWidth - edgeMargin && attempts < maxSearchAttempts;
          right += searchStepX
        ) {
        ...
        logger.debug(
          ` 使用最佳位置: right=${bestPosition.right}, top=${bestPosition.top}, overlap=${bestPosition.overlapRatio}`
        )
      ...
      detectPageLayout() {
        ...
        logger.debug(` 页面布局检测:`, {
          element:
            mainElement.tagName +
            (mainElement.className ? '.' + mainElement.className.split(' ')[0] : ''),
      ...
      findBestIconPosition(preferredTop, preferredRight) {
        ...
        const {
          ...
          iconHeight,
          edgeMargin,
          iconPositionSearchSteps,
          iconPositionSearchStepY,
        } = this.config
        const scrollbarWidth = this.getScrollbarWidth()
      ...
      findBestPanelPosition(panelEl, preferredRight, preferredTop, width, height) {
        ...
        // 1. 评估首选位置
        const preferredScore = this.evaluatePanelPosition(
          preferredRight,
          preferredTop,
          width,
          height,
        ...
        if (preferredScore.severity !== 'none') {
          logger.debug(
            ` panel首选位置有遮挡: severity=${preferredScore.severity}, ratio=${(preferredScore.overlapRatio * 100).toFixed(1)}%, elements=${preferredScore.elementCount}`
          )
        }
        ...
        // 向左搜索（right增大）
        for (
          let right = preferredRight + searchStepX;
          right <= vw - width - edgeMargin && attempts < maxSearchAttempts;
          right += searchStepX
        ) {
          const score = this.evaluatePanelPosition(
            right,
            preferredTop,
            width,
            height,
        ...
        // 向上搜索
        for (
          let top = preferredTop - searchStepY;
          top >= edgeMargin && attempts < maxSearchAttempts;
          top -= searchStepY
        ) {
          const score = this.evaluatePanelPosition(
            preferredRight,
        ...
        for (
          let top = preferredTop + searchStepY;
          top <= vh - height - edgeMargin && attempts < maxSearchAttempts;
          top += searchStepY
        ) {
          const score = this.evaluatePanelPosition(
            preferredRight,
        ...
        for (const compReport of report.components) {
          ...
          if (compReport.panel && compReport.panel.hasOcclusion && !this.collapsedStates[c.id]) {
            ...
            if (panelBounds) {
              ...
              if (
                bestPos.severity !== 'critical' &&
                bestPos.overlapRatio < compReport.panel.overlapRatio
              ) {
