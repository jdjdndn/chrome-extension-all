...
const REDIRECT_LINKS_CONFIG = {
  ...
  // 目标URL参数名
  TARGET_PARAMS: ['target', 'url', 'to', 'u', 'link', 'href', 'q', 'redirect', 'goto', 'jump', 'next', 'dest', 'out', 'return', 'back'],
  // Hash中的参数名
...
function initRedirectLinks() {
  ...
  const logger = window.LoggerUtils
    ...
    : { debug: () => {}, info: (...a) => console.log('[redirect-links]', ...a), warn: (...a) => console.warn('[redirect-links]', ...a), error: (...a) => console.error('[redirect-links]', ...a) };
  ...
  const { LINK_PATTERNS, TARGET_PARAMS, HASH_PARAMS, SKIP_DOMAINS, PROCESSED_ATTR } = REDIRECT_LINKS_CONFIG;
  ...
  function processLinks() {
    ...
    links.forEach(link => {
      ...
      // 白名单域名跳过替换
      if (SKIP_DOMAINS.some(domain => link.hostname === domain || link.hostname.endsWith('.' + domain))) return;
      ...
      if (realUrl && realUrl !== originalHref) {
        link.href = realUrl;
        replacedCount++;
        logger.debug('替换重定向链接:', originalHref.substring(0, 60) + '...', '->', realUrl.substring(0, 60) + '...');
      }
    });
    ...
    if (replacedCount > 0) {
      ...
      try {
        ...
        chrome.storage.local.get(key, result => {
          ...
          chrome.storage.local.set({ [key]: { total: prev.total + replacedCount, sessions: prev.sessions + 1 } });
        });
      } catch (e) { /* 非扩展环境或存储不可用 */ }
    }
  }
