...
(async function() {
  ...
  class BiliSite extends SiteBase {
    ...
    refreshHideSelectors() {
      ...
      console.log('[Bilibili脚本] 已生成隐藏选择器，关键词数量:', this.keywords.notInterested.length);
    }
    ...
    async loadKeywords() {
      ...
      if (this.state.localServerAvailable) {
        ...
        if (serverData?.notInterested?.length > 0) {
          ...
          console.log('[Bilibili脚本] 从本地服务加载关键词:', this.keywords.notInterested.length, '个');
          return;
        }
      }
      ...
      if (result.biliKeywords?.notInterested) {
        ...
        console.log('[Bilibili脚本] 从Chrome存储加载关键词:', this.keywords.notInterested.length, '个');
      }
    }
  ...
  window.BiliScriptConfig = {
    get DEFAULT_HIDE_SELECTORS() { return biliSite.defaultSelectors; },
    BLOCKED_DOMAINS: [],
    get NOT_INTERESTED_KEYWORDS() { return biliSite.keywords.notInterested; },
    DEFAULT_NOT_INTERESTED_KEYWORDS: [],
    FEED_CARD_SELECTORS: biliSite.feedCardSelectors,
    get localServerAvailable() { return biliSite.state.localServerAvailable; }
  };
  ...
  biliSite.createMessageHandler({
    'UPDATE_KEYWORDS': (message) => {
      ...
      if (keywords.NOT_INTERESTED_KEYWORDS) {
        ...
        console.log('[Bilibili脚本] 不感兴趣关键词已更新:', biliSite.keywords.notInterested.length, '个');
        updated = true;
      }
    ...
    'SYNC_TO_SERVER': async () => {
      ...
      const selectorsSaved = await biliSite.saveToServer('selectors', biliSite.state.currentSelectors);
      return {
  ...
  async function init() {
    ...
    console.log('[Bilibili脚本] 初始化完成，本地服务:', biliSite.state.localServerAvailable ? '已连接' : '未连接');
