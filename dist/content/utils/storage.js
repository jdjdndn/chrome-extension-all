...
(function () {
  ...
  async function getSync(keys) {
    ...
    if (isStorageBridgeReady()) {
      const result = await StorageBridge.get(keys, 'sync');
      return result || {};
    }
  ...
  async function setSync(data) {
    ...
    // 优先使用 StorageBridge
    if (isStorageBridgeReady()) {
      return await StorageBridge.set(data, 'sync');
    }
  ...
  async function getLocal(keys) {
    ...
    // 优先使用 StorageBridge
    if (isStorageBridgeReady()) {
      const result = await StorageBridge.get(keys, 'local');
      return result || {};
    }
  ...
  async function setLocal(data) {
    ...
    // 优先使用 StorageBridge
    if (isStorageBridgeReady()) {
      return await StorageBridge.set(data, 'local');
    }
  ...
  async function remove(keys, area = 'local') {
    ...
    // 优先使用 StorageBridge
    if (isStorageBridgeReady()) {
      return await StorageBridge.remove(keys, area);
    }
   ...
   */
  function watch(key, callback, area = 'local') {
    if (isStorageBridgeReady()) {
      return StorageBridge.watch(key, callback, area);
    }
  ...
  function subscribe(callback) {
    if (isStorageBridgeReady()) {
      return StorageBridge.subscribe(callback);
    }
   ...
   */
  async function getDomainSettings(settingsKey, domain) {
   ...
   * @returns {Promise<boolean>}
   */
  async function setDomainSettings(settingsKey, domain, settings) {
   ...
   * @returns {Promise<object>}
   */
  async function reactive(key, defaultValue = null, area = 'local') {
    if (isStorageBridgeReady()) {
      return await StorageBridge.reactive(key, defaultValue, area);
    }
