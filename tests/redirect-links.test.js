...
(function () {
  ...
  const RedirectLinksTest = {
    ...
    testConfigExists() {
      ...
      this.assert(Array.isArray(REDIRECT_LINKS_CONFIG.SKIP_DOMAINS), 'SKIP_DOMAINS 是数组')

      this.assert(typeof window.RedirectLinksUtils !== 'undefined', 'RedirectLinksUtils 已导出')
    },
    ...
    testDecodeUrlValue() {
      ...
      const testCases = [
        ...
        {
          input: 'https%3A%2F%2Fexample.com%3Fkey%3Dvalue',
          expected: 'https://example.com?key=value',
        },
        { input: 'https://example.com', expected: 'https://example.com' },
        {
          input: 'https%3A%2F%2Fexample.com%2Fpath%3Fa%3D1%26b%3D2',
    ...
    testExtractTargetUrl() {
      ...
      const testCases = [
        // 标准 target 参数
        {
          input: 'https://link.zhihu.com/?target=https://example.com',
          expected: 'https://example.com',
        },
        // url 参数
        {
          input: 'https://example.com/redirect?url=https://target.com',
          expected: 'https://target.com',
        },
        // to 参数
        ...
        // 新增 return 参数
        {
          input: 'https://example.com/go?return=https://return.com',
          expected: 'https://return.com',
        },
        // 新增 back 参数
        { input: 'https://example.com/go?back=https://back.com', expected: 'https://back.com' },
        // 嵌套编码
        {
          input: 'https://example.com/go?target=https%3A%2F%2Ftarget.com',
          expected: 'https://target.com',
        },
        // hash 中的参数
        {
          input: 'https://example.com/page#target=https://hash-target.com',
          expected: 'https://hash-target.com',
        },
        // 无效 URL
        { input: 'not-a-url', expected: null },
        // 空值
    ...
    testLinkPatterns() {
      ...
      this.assert(
        patterns.length >= 9,
        `LINK_PATTERNS 包含至少 9 个模式（实际 ${patterns.length}）`
      )

      const stringPatterns = patterns.filter((p) => typeof p === 'string')
      this.assert(
        stringPatterns.length >= 5,
        `包含至少 5 个字符串模式（实际 ${stringPatterns.length}）`
      )

      const regexPatterns = patterns.filter((p) => p instanceof RegExp)
      this.assert(
        regexPatterns.length >= 3,
        `包含至少 3 个正则模式（实际 ${regexPatterns.length}）`
      )
      ...
      if (zhihuRegex) {
        ...
        this.assert(!zhihuRegex.test('https://zhihu.com/hot'), '知乎热榜链接不匹配')
      }

      // 抖音正则
      const douyinRegex = regexPatterns.find((r) => r.source.includes('douyin'))
      if (douyinRegex) {
        this.assert(douyinRegex.test('https://douyin.com/user/123456'), '抖音用户链接正则匹配')
      }
      ...
      if (biliRegex) {
        ...
        this.assert(!biliRegex.test('https://www.bilibili.com/anime/'), 'B站动画页面不匹配')
      }

      // 小红书正则
      const xhsRegex = regexPatterns.find((r) => r.source.includes('xhslink'))
      if (xhsRegex) {
        this.assert(xhsRegex.test('https://xhslink.com/a/abc123'), '小红书短链接正则匹配')
      }

      // 微博字符串模式
      this.assert(patterns.includes('weibo.com/n/'), '包含微博字符串模式')

      // 百度贴吧字符串模式
      this.assert(patterns.includes('tieba.baidu.com/p/'), '包含百度贴吧字符串模式')
    },
    ...
    testSkipDomains() {
      ...
      this.assert(
        skipDomains.length >= 3,
        `SKIP_DOMAINS 包含至少 3 个域名（实际 ${skipDomains.length}）`
      )
    },
  }
