...
(function () {
  ...
  const EventBusV46Test = {
    ...
    async runAll() {
      ...
      this.assert(typeof EventBus.Transport.registerPort === 'function', 'Transport.registerPort 方法存在');
      this.assert(typeof EventBus.Transport.broadcast === 'function', 'Transport.broadcast 方法存在');
      ...
      this.assert(typeof EventBus.Chrome.isExtensionContext === 'function', 'Chrome.isExtensionContext 方法存在');
      this.assert(typeof EventBus.Chrome.sendToActiveTab === 'function', 'Chrome.sendToActiveTab 方法存在');
      this.assert(typeof EventBus.Chrome.sendToContent === 'function', 'Chrome.sendToContent 方法存在');
      this.assert(typeof EventBus.Chrome.broadcast === 'function', 'Chrome.broadcast 方法存在');
      this.assert(typeof EventBus.Chrome.getCurrentTabId === 'function', 'Chrome.getCurrentTabId 方法存在');
      ...
      this.assert(typeof MessagingUtils.sendToBackground === 'function', 'MessagingUtils.sendToBackground 已定义');
      ...
      this.assert(typeof MessagingUtils.waitForEventBus === 'function', 'MessagingUtils.waitForEventBus 已定义');
      ...
      this.assert(typeof ContentBridge.sendMessage === 'function', 'ContentBridge.sendMessage 已定义');
