...
(function () {
  ...
  const EventBusTest = {
    ...
    async testTransportLayer() {
      ...
      if (EventBus.Transport) {
        ...
        this.assert(typeof EventBus.Transport.sendViaPort === 'function', 'Transport.sendViaPort 方法存在');
      }
    ...
    async testMessagingUtilsIntegration() {
      ...
      if (typeof MessagingUtils !== 'undefined') {
        this.assert(typeof MessagingUtils.sendToBackground === 'function', 'sendToBackground 方法存在');
        this.assert(typeof MessagingUtils.isEventBusReady === 'function', 'isEventBusReady 方法存在');
        this.assert(typeof MessagingUtils.waitForEventBus === 'function', 'waitForEventBus 方法存在');
  ...
  console.log('[EventBus集成测试] 测试脚本已加载。运行 EventBusIntegrationTest.runAllTests() 开始测试。');
})();
