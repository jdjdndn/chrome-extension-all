...
(function () {
  ...
  const ResourceAcceleratorTest = {
    ...
    async testCDNMappings() {
      ...
      this.assert(jqueryResult && jqueryResult.name === 'jquery', 'jQuery URL匹配正确')
      this.assert(jqueryResult && jqueryResult.version === '3.7.1', 'jQuery版本提取正确')

      const reactResult = window.CDNMappings.matchJSLibrary(
        'https://unpkg.com/react@18.2.0/umd/react.production.min.js'
      )
      this.assert(reactResult && reactResult.name === 'react', 'React URL匹配正确')

      const vueResult = window.CDNMappings.matchJSLibrary(
        'https://cdn.jsdelivr.net/npm/vue@3.4.21/dist/vue.global.min.js'
      )
      this.assert(vueResult && vueResult.name === 'vue', 'Vue URL匹配正确')

      // 测试智能匹配 - 已知包名模式
      const smartMatch = window.CDNMappings.matchJSLibrary(
        'https://example.com/lodash@4.17.21/lodash.min.js'
      )
      this.assert(smartMatch && smartMatch.isAutoDetected === true, '智能匹配识别 lodash 包')
      ...
      try {
        ...
        this.assert(asyncMatch !== null, '异步匹配 ramda 包成功')
      } catch (e) {
      ...
      this.assert(fontResult && fontResult.name === 'google-fonts', 'Google Fonts URL匹配正确')
      this.assert(fontResult && fontResult.cdnUrl.includes('fonts.font.im'), '字体镜像URL替换正确')
    },
    ...
    async testJSReplacer() {
      ...
      this.assert(replacer.enabled === true, 'JSReplacer初始化正确')

      // 测试启用/禁用
      replacer.disable()
      this.assert(replacer.enabled === false, 'JSReplacer禁用成功')

      replacer.enable()
      this.assert(replacer.enabled === true, 'JSReplacer启用成功')

      // 测试统计
      const stats = replacer.getStats()
      this.assert(typeof stats === 'object' && 'replaced' in stats, 'JSReplacer统计格式正确')
    },
    ...
    async testFontReplacer() {
      ...
      this.assert(replacer.enabled === true, 'FontReplacer初始化正确')

      // 测试启用/禁用
      replacer.disable()
      this.assert(replacer.enabled === false, 'FontReplacer禁用成功')

      replacer.enable()
      this.assert(replacer.enabled === true, 'FontReplacer启用成功')
    },
    ...
    async testImageOptimizer() {
      ...
      this.assert(optimizer.lazyLoadEnabled === true, 'ImageOptimizer懒加载初始化正确')
      this.assert(optimizer.compressEnabled === true, 'ImageOptimizer压缩初始化正确')
      this.assert(optimizer.compressQuality === 0.8, 'ImageOptimizer压缩质量设置正确')

      // 测试WebP支持检测
      const supportsWebP = optimizer.supportsWebP()
      this.assert(typeof supportsWebP === 'boolean', 'WebP支持检测返回布尔值')

      // 测试启用/禁用
      optimizer.disableLazyLoad()
      this.assert(optimizer.lazyLoadEnabled === false, '懒加载禁用成功')

      optimizer.enableLazyLoad()
      this.assert(optimizer.lazyLoadEnabled === true, '懒加载启用成功')
    },
    ...
    async testResourceAccelerator() {
      ...
      this.assert(accelerator.config.enabled === true, 'ResourceAccelerator初始化正确')

      // 测试配置更新
      accelerator.config.imageQuality = 0.9
      this.assert(accelerator.config.imageQuality === 0.9, '配置更新成功')
      ...
      // v7: Site config
      this.assert(typeof accelerator.config.siteConfig === 'object', 'siteConfig config exists')
      this.assert(
      ...
      this.assert(typeof accelerator.exportConfig === 'function', 'exportConfig function exists')
      this.assert(typeof accelerator.importConfig === 'function', 'importConfig function exists')
      this.assert(typeof accelerator.resetConfig === 'function', 'resetConfig function exists')
  ...
  console.log(
    '[ResourceAcceleratorTest] 测试模块已加载，运行 ResourceAcceleratorTest.runAll() 开始测试'
  )
})()
